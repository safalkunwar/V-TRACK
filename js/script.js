...
async function nearBusStop () {
  ...
  try {
    ...
    if (busStops.length > 0) {
      ...
      // Display the first few bus stops on the map
      for (let i = 0; i < 5 && i < busStops.length; i++) {
        const busStop = busStops[i];
        const marker = new google.maps.Marker ({
          position: {lat: busStop.lat, lng: busStop.lng},
          map: map,
          title: `Bus Stop ${busStop.id.toUpperCase ()} 🚌`,
          icon: 'http://maps.google.com/mapfiles/ms/icons/green-dot.png',
        });
      }
    } else {
      alert ('No bus stops found near you.');
    }
  } catch (error) {
...
async function fetchBusStops (userLocation) {
  const url = `https://maps.googleapis.com/maps/api/geocode/json?address=${userLocation.lat},${userLocation.lng}&key=YOUR_API_KEY`;
